...
const AUTH_KEY = 'doit.auth.v3';
const DB_KEY = 'doit.db.v3';
const SETTINGS_KEY = 'doit.settings.v3';
...
function saveJson(key: string, value: unknown) { if (typeof window !== 'undefined') window.localStorage.setItem(key, JSON.stringify(value)); }
...
function parseWorksheet(xml: string, sharedStrings: string[], name: string): { name: string; headers: string[]; rows: Row[] } {
  ...
  for (const row of Array.from(doc.getElementsByTagName('row'))) {
    ...
    for (const cell of Array.from(row.getElementsByTagName('c'))) {
      ...
      else { const raw = safeText(v?.textContent); value = raw === '' ? '' : Number.isFinite(Number(raw)) ? Number(raw) : raw; }
      matrix[ri][ci] = value;
    }
  }
...
async function parseCsvFile(file: File, profile: Profile): Promise<Workbook> {
  ...
  return rebuildWorkbook(file.name, new Date().toISOString(), [{ ...parsed, mapping }], ['CSV fallback mode'], profile);
}

export default function DoitAppLean() {
  ...
  useEffect(() => {
    if (!workbook) return;
    setWorkbook((current) => current ? rebuildWorkbook(current.sourceName, current.parsedAt, current.sheets.map((s) => ({ name: s.name, headers: s.headers, rows: s.rows, mapping: s.mapping })), current.warnings, profile) : current);
  }, [profile]);
  ...
  function persistWorkbook(next: Workbook) {
    setWorkbook(next);
    setDB((prev) => ({ ...prev, workbooks: [{ name: next.sourceName, parsedAt: next.parsedAt, sheets: next.sheets.length, rows: next.rowCount, anomalies: next.anomalyCount }, ...prev.workbooks].slice(0, 20), reports: [{ title: next.sourceName, generatedAt: next.parsedAt, markdown: next.reportMarkdown, source: next.sourceName }, ...prev.reports].slice(0, 20) }));
  }
  ...
  async function handleLoadFile(file: File) {
    ...
    try {
      ...
      persistWorkbook(wb); setActiveSheet(0); setView('dashboard'); setStatus(`โหลดสำเร็จ: ${wb.sheets.length} sheet`);
    } catch (e) { setWorkbook(null); setError((e as Error).message || 'ไม่สามารถอ่านไฟล์ได้'); setStatus('เกิดข้อผิดพลาด'); }
  }

  async function handleImportDatabase(file: File) {
    if (!session) { setError('กรุณาเข้าสู่ระบบก่อน'); return; }
    try { setDB(parseDatabaseFile(await file.text())); setStatus('นำเข้าฐานข้อมูลสำเร็จ'); } catch (e) { setError((e as Error).message || 'ไม่สามารถนำเข้าฐานข้อมูลได้'); }
  }
  ...
  function updateMapping(field: MappingKey, header: string) {
    ...
    const next = rebuildWorkbook(workbook.sourceName, workbook.parsedAt, nextSheets.map((s) => ({ name: s.name, headers: s.headers, rows: s.rows, mapping: s.mapping })), workbook.warnings, profile);
        ...
        {!session ? (
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 16 }}>
              {(['dashboard', 'sheets', 'relations', 'tod', 'reports', 'database', 'settings'] as View[]).map((v) => <button key={v} style={btn(v)} onClick={() => setView(v)}>{v}</button>)}
              <button style={btn('dashboard')} onClick={exportReport} disabled={!workbook}>Export MD</button>
              <button style={btn('dashboard')} onClick={exportJson} disabled={!workbook}>Export JSON</button>
              <button style={btn('database')} onClick={logout}>Logout</button>
            </div>
            ...
            {workbook && view === 'sheets' && active && (
              <div style={{ marginTop: 22, display: 'grid', gap: 18 }}>
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>{workbook.sheets.map((sheet, idx) => <button key={sheet.name + idx} onClick={() => setActiveSheet(idx)} style={{ ...btn('sheets'), background: idx === activeSheet ? `${t.accent}22` : t.panel }}>{sheet.name}</button>)}</div>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ ...panel, padding: 18 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
                        ...
                        <div style={{ color: t.muted }}>{active.rows.length} rows · {active.headers.length} columns · {summarizeHeaders(active.headers)}</div>
                      </div>
            ...
            {view === 'database' && <div style={{ marginTop: 22, display: 'grid', gap: 18 }}><div style={{ ...panel, padding: 18 }}><div style={{ fontWeight: 800, fontSize: 18 }}>Persistent local database</div><div style={{ color: t.muted, marginTop: 4 }}>Sessions, workbooks, reports and templates are saved in browser storage.</div><div style={{ display: 'grid', gap: 10, marginTop: 12 }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: t.muted }}>Sessions</span><strong>{db.sessions.length}</strong></div><div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: t.muted }}>Workbooks</span><strong>{db.workbooks.length}</strong></div><div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: t.muted }}>Reports</span><strong>{db.reports.length}</strong></div><div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ color: t.muted }}>Templates</span><strong>{Object.keys(db.templates).length}</strong></div></div><div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 14 }}><button onClick={handleExportDatabase} style={btn('database')}>Export DB</button><label style={{ cursor: 'pointer', padding: '10px 14px', borderRadius: 12, border: `1px solid ${t.border}`, background: t.panel, color: t.text, fontWeight: 700 }}>
                      Import DB
                      <input type="file" accept="application/json,.json" style={{ display: 'none' }} onChange={(e) => { const f = e.target.files?.[0]; if (f) void handleImportDatabase(f); }} />
                    </label><button onClick={() => { setDB(DEFAULT_DB); setStatus('Database cleared'); }} style={btn('database')}>Clear DB</button></div></div><div style={{ ...panel, padding: 18 }}><div style={{ fontWeight: 800, fontSize: 18 }}>Session</div><div style={{ marginTop: 10 }}>User: <strong>{session.name}</strong></div><div>Org: <strong>{session.org}</strong></div><div>Token: <code>{session.token.slice(0, 8)}...</code></div></div></div>}

            {view === 'settings' && <div style={{ marginTop: 22, ...panel, padding: 18 }}><div style={{ fontWeight: 800, fontSize: 18 }}>Settings & scoring profiles</div><div style={{ display: 'grid', gap: 16, marginTop: 14, maxWidth: 760 }}><label style={{ display: 'grid', gap: 8, padding: 14, borderRadius: 14, background: t.card, border: `1px solid ${t.border}` }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Dark mode</span><strong>{String(dark)}</strong></div><input type="checkbox" checked={dark} onChange={(e) => setDark(e.target.checked)} /></label><label style={{ display: 'grid', gap: 8, padding: 14, borderRadius: 14, background: t.card, border: `1px solid ${t.border}` }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Rows shown</span><strong>{rowsLimit}</strong></div><input type="range" min={25} max={500} step={25} value={rowsLimit} onChange={(e) => setRowsLimit(Number(e.target.value))} /></label><label style={{ display: 'grid', gap: 8, padding: 14, borderRadius: 14, background: t.card, border: `1px solid ${t.border}` }}><div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Scoring profile</span><strong>{profile}</strong></div><select value={profile} onChange={(e) => setProfile(e.target.value as Profile)} style={{ padding: 12, borderRadius: 12, border: `1px solid ${t.border}`, background: t.panel, color: t.text }}><option value="balanced">balanced</option><option value="strict">strict</option><option value="forensic">forensic</option></select></label></div></div>}
          </>
        )}
      </div>
    </div>
