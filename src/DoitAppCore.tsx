...
type SheetData = RawSheet & {
  anomalies: RowAnalysis[];
  numericSummary: ReturnType<typeof numericSummary>;
  storeSummary: StoreStat[];
  todSummary: TodStat[];
};
type Workbook = {
  sourceName: string;
...
function rebuildSheet(sheet: RawSheet, profile: Profile): SheetData {
  ...
  return {
    ...
    numericSummary: numericSummary(sheet.rows),
...
function rebuildWorkbook(sourceName: string, parsedAt: string, sheets: RawSheet[], warnings: string[], profile: Profile): Workbook {
  const computed = sheets.map((sheet) => rebuildSheet(sheet, profile));
  ...
  const wb = { sourceName, parsedAt, sheets: computed, warnings, warningCount: warnings.length, rowCount, anomalyCount, relations, reportMarkdown: '' };
  wb.reportMarkdown = buildReport(wb);
  return wb;
}
...
async function parseWorkbookFile(file: File, db: DB, profile: Profile): Promise<Workbook> {
  ...
  for (const ref of refs) {
    ...
    if (!xml) {
      warnings.push(`Missing sheet data: ${ref.name}`);
      continue;
    }
...
export default function DoitAppCore() {
  ...
  function updateMapping(field: CanonicalField, header: string) {
    ...
    persistWorkbook(rebuildWorkbook(workbook.sourceName, workbook.parsedAt, nextSheets.map((s) => ({ name: s.name, headers: s.headers, rows: s.rows, mapping: s.mapping })), workbook.warnings, profile));
        ...
        {!session ? (
          ...
          <>
            ...
            <div style={{ marginTop: 18, ...panel, padding: 18 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <label style={{ cursor: 'pointer', padding: '12px 16px', borderRadius: 14, background: t.accent, color: '#082f49', fontWeight: 800 }}>
                  Choose file
                  <input type="file" accept=".xlsx,.xlsm,.zip,.csv,.txt" style={{ display: 'none' }} onChange={(e) => { const f = e.target.files?.[0]; if (f) void loadFile(f); }} />
                </label>
              </div>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 16 }}>
              <button style={styleBtn('dashboard')} onClick={() => setView('dashboard')}>Dashboard</button>
              <button style={styleBtn('sheets')} onClick={() => setView('sheets')}>Sheets</button>
              <button style={styleBtn('relations')} onClick={() => setView('relations')}>Relations</button>
              <button style={styleBtn('tod')} onClick={() => setView('tod')}>TOD / Store</button>
              <button style={styleBtn('reports')} onClick={() => setView('reports')}>Reports</button>
              <button style={styleBtn('database')} onClick={() => setView('database')}>Database</button>
              <button style={styleBtn('settings')} onClick={() => setView('settings')}>Settings</button>
              ...
              <button style={styleBtn('dashboard')} onClick={logout}>Logout</button>
            </div>
            ...
            {view === 'database' && (
              <div style={{ marginTop: 22, display: 'grid', gap: 18 }}>
                <div style={{ ...panel, padding: 18 }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginTop: 14 }}><button onClick={() => downloadJson('doit-database.json', db)} style={styleBtn('database')}>Export DB</button><button onClick={() => { setDB(DEFAULT_DB); setStatus('Database cleared'); }} style={styleBtn('database')}>Clear DB</button></div>
                </div>
                <div style={{ ...panel, padding: 18 }}>
